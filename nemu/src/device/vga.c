...
static uint32_t *vgact1_port_base = NULL;
...
void vga_update_screen(){ 
  ...
  //then zero out the sync regisrer
  #ifdef SHOW_SCREEN
  if (vgact1_port_base[1] != 0){
    update_screen();
    vgact1_port_base[1]= 0;
  }
  #endif
}
...
void init_vga() {
 ...
 vgact1_port_base = (void*) new_space(8);
 vgact1_port_base[0]=((SCREEN_W)<<16 | (SCREEN_H));
 vgact1_port_base[1] = 1;
 add_pio_map("screen",VGACTL_PORT,(void*)vgact1_port_base,8,NULL);
 add_mmio_map("screen",VGACTL_MMIO,(void*)vgact1_port_base,8,NULL);
