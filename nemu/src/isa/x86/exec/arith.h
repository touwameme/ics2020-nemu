...
static inline def_EHelper(das){
  uint8_t al = cpu.eax&0xff;
  if((al & 0xf) >9){
    cpu.eax = (cpu.eax & 0xffffff00) + al - 6;
  }
  if ((al > 0x9f ) ||  (cpu.eflags&EFLAG_CF)!= 0){
    cpu.eax = (cpu.eax & 0xffffff00 )+ al - 0x60;
    cpu.eflags |= EFLAG_CF;
...
static inline def_EHelper(cmp) {
  ...
  *s2 = *dsrc1;
  rtl_sub(s,s0,s1,s2);
  rtl_is_sub_carry(s,t0,s1,s2);
  rtl_set_CF(s, t0);
  rtl_is_sub_overflow(s,t0,s0,s1,s2,id_dest->width);
  rtl_set_OF(s, t0);
  rtl_update_ZFSF(s,s0,id_dest->width);
...
static inline def_EHelper(inc) {
  ...
  *s1 = (*s0== 0x80000000);
  rtl_set_OF(s, s1);
  //rtl_is_sub_overflow(s,&cpu.eflags,&v,ddest,dsrc1,id_dest->width);
  rtl_update_ZFSF(s,s0,id_dest->width);
 ...
 *s2 = *s0;
 ...
 uint32_t v = -1;
 if(id_dest->width != 4){
   rtl_andi(s,s1,s1,v>>(4-8*(id_dest->width)));
 ...
 rtl_or(s,s0,s0,s1);
 rtl_is_sub_overflow(s,s0,s2,&id_dest->val,&id_src1->val,id_dest->width);
 rtl_set_OF(s,s0);
